...
contract product {
    ...
    function queryProductsList(bytes32 _sellerCode) public view returns(
        ...
        brands = new bytes32[](len);
        prices = new uint256[](len);
        statuses = new bytes32[](len);
        for(uint i=0;i<len;i++){
            bytes32 sn = productSNs[i];
            uint256 pid = productMap[sn];
            if(pid < productCount && productItems[pid].productSN == sn){
                productItem memory item = productItems[pid];
                ids[i] = item.productId;
                sns[i] = item.productSN;
                names[i] = item.productName;
                brands[i] = item.productBrand;
                prices[i] = item.productPrice;
                statuses[i] = item.productStatus;
            }
        }
        return(ids, sns, names, brands, prices, statuses);
    }

    function getProductsForSeller(bytes32 _sellerCode) public view returns(bytes32[] memory) {
